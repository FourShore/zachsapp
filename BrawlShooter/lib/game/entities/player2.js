...
EntityPlayer2 = ig.Box2DEntity.extend({
	size: {x: 8, y:14},
	offset: {x: 4, y: 2},
	MaxHealth: 30,
	health: 30,
	...
	update: function() {
		...
		// shoot
		if( ig.input.pressed('shoot2') ) {
			var x = this.pos.x + (this.flip ? -6 : 6 );
			var y = this.pos.y + 6;
			ig.game.spawnEntity( EntityProjectile2, x, y+4, {flip:this.flip} );
		}
...
EntityProjectile2 = ig.Box2DEntity.extend({
	size: {x: 8, y: 4},
	offset: {x: 1, y: 1},
	playerId: 2,
	entId: 1,
...
EntityHealthBar2 = ig.Box2DEntity.extend({
